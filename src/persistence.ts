...
export function jobFolder(
  jobId: string,
  bucket: "queued" | "in-progress" | "completed" | "failed",
): string {
...
export async function moveJobFolder(
  jobId: string,
  fromBucket: string,
  toBucket: string,
): Promise<string> {
  ...
  try {
    ...
    /* ignore if job.json can't be read */
  }

  return newInputPath;
}

export async function saveRemapData(
  jobId: string,
  remapData: RemapData,
): Promise<void> {
  ...
  await atomicWrite(
    path.join(folder, "remapdata.json"),
    ...
    try {
      ...
      for (const entry of dir) {
        ...
        try {
          ...
          console.warn(
            `[persistence] Skipping corrupt/missing job.json in ${jobDir}`,
          );
        }
      }
    } catch {
...
export async function saveEntityCache(
  ...
  await atomicWrite(
    path.join(cacheDir, "entity-cache.json"),
